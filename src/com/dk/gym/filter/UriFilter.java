...
public class UriFilter implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain) throws ServletException, IOException {
        ...
        if (urlQuery.matches(URI_REGEX)) {
            ...
            uri = (uri.length() == 1 && uri.startsWith("/")) ? INDEX_PAGE_COMMAND_VALUE : uri.substring(1, uri.length()).replace('/', '.').toLowerCase();
            LOGGER.log(Level.DEBUG, "urlQuery: " + urlQuery);
            LOGGER.log(Level.DEBUG, "Command uri: " + uri);
            req.setAttribute(ParamConstant.PARAM_COMMAND, uri);
